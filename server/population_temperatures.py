...
temp_dates = pd.read_csv("Temperature Data.csv", usecols = ["location_date"])
# print(temp_dates.values.min())
minDate = datetime.strptime(temp_dates.values.min(), '%m/%d/%Y').date()
maxDate = datetime.strptime(temp_dates.values.max(), '%m/%d/%Y').date()
maxDate -= timedelta(days=1)
maxDate += timedelta(days=1)
date_range = pd.date_range(minDate, maxDate)
# date_range = date_range.union([date_range[-1] + 1]).strftime('%Y-%m')
# date_range
...
# print('Intercept2: \n', intercept2)
# print('Coefficients2: \n', slope2)
population_weighted = population_weighted.assign(temp_min_c = lambda x: x["temp_min_c"] - x["population"]*slope1[0])
...
# print('Intercept3: \n', intercept3)
# print('Coefficients3: \n', slope3)
population_weighted = population_weighted.assign(temp_max_c = lambda x: x["temp_max_c"] - x["population"]*slope1[0])


# In[12]:


final_data = pd.concat([population_weighted, nan_cities])
